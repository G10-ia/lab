document.addEventListener("DOMContentLoaded", () => {
  ...
  function obtenerHtmlDetalleActividad(item) {
    ...
    return `
      <button type="button" class="btn-actividad-detalle ${claseBotonEstado}" data-user-id="${escapeHtml(claveActividad)}">${textoBoton}</button>
      ${detalleHtml}
    `;
  }
  ...
  function actualizarResumenGeneralAdmin() {
    ...
    const totalUsuarios = Array.isArray(usuariosAdminOriginalCache) ? usuariosAdminOriginalCache.filter((u) => u && u.activo === true).length : 0;
    const totalAccesos = Array.isArray(actividadAdminCache) ? actividadAdminCache.reduce((s, item) => s + Number(item?.contador_ingresos || 0), 0) : 0;
    const ultimo = Array.isArray(actividadAdminCache) && actividadAdminCache.length ? (actividadAdminCache[0].usuario || "-") : "-";
    ...
    listaActividadAdmin.innerHTML = actividadFiltrada.map((item) => {
      ...
      return `
        <div class="tarjeta-actividad-admin">
          ...
          <div class="tarjeta-actividad-admin-lineas">
            <div class="actividad-linea"><span>${tipoUltimoEvento}</span><strong>${fechaUltimoEvento}</strong></div>
            <div class="actividad-linea"><span>Resumen:</span><strong>${etiquetaIngresos} · ${etiquetaFallidos}</strong></div>
          </div>
